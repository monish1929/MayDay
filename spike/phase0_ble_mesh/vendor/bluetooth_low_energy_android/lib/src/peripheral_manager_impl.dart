...
final class PeripheralManagerImpl
  ...
  Future<void> startAdvertising(Advertisement advertisement) async {
    ...
    // PATCHED for MayDay Phase 0 — spike/phase0_ble_mesh/PHASE0_MESH_FINDINGS.md
    // §9. Upstream never sets txPowerLevelArgs, leaving Android's advertiser
    // at ADVERTISE_TX_POWER_MEDIUM by default even though the native side
    // (BluetoothLowEnergyApi.kt toAdvertiseSettings()) fully supports HIGH —
    // confirmed by reading the Kotlin source before patching. This is a
    // vendored local copy (see pubspec.yaml dependency_overrides), not a
    // real fork; do not let this survive into lib/mesh/ without deciding
    // whether MayDay should always advertise at max power (battery
    // trade-off) as a deliberate product choice, not a silent default.
