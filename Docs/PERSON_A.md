...
- [~] Effective range outdoors, line of sight — pre-patch informal check reached ~40m (§10). Two more informal points since, both on the TX-HIGH build: discovery persisting (badly) to ~100m as a Day 3 byproduct (§12), and write range separately estimated around ~50m from RSSI correlation across a session (§13). All three are informal — none walked or marked to a known distance. **Still needs a real Day 4 re-run:** walk apart with distance actually measured (tape/GPS/paced steps), log RSSI and write success at fixed markers, on the current TX-HIGH build. See `PHASE0_MESH_FINDINGS.md` §10, §12, §13
...
| 2026-08-20 | `a/ble-mesh-spike` | First real three-phone Day 3 attempt, all 3 devices (§6 fix means the Motorola block is gone). **Inconclusive, not a pass:** middle phone (B) never had Scan on, so `_peers` stayed empty on B and relay was a structural no-op regardless of the relay toggle — nothing was actually forwarded, and neither A nor C logged an `RX ... hops=1`. Root cause is a gap in the Day 3 protocol itself (never told the middle node to enable Scan), not a mesh or plugin bug — fixed in `spike/phase0_ble_mesh/README.md` Day 3 step 2. Full trace in `PHASE0_MESH_FINDINGS.md` §12. Side finding: C independently observed the same BLE-address-rotation behavior as §7, from the scanning side this time (two distinct `FOUND A` events, different peripheral UUIDs, 499s apart). | **Day 3 still not done.** Protocol gap is fixed; needs one clean re-run with all three of B's toggles (Advertise/Scan/relay) confirmed live before sending. |
| 2026-08-20 | `a/ble-mesh-spike` | Second session, two phones (A/B), outdoor line-of-sight, ~50m by informal post-hoc estimate. Writes were unreliable early (RSSI -90 to -95, mostly `TIMED OUT`) and increasingly reliable as RSSI improved to -55/-56 later — write range visibly shorter than discovery range, on the TX-HIGH build, with RSSI correlation across a whole session rather than one failed attempt. Confirms §10's mechanism (discovery is one-way repeated broadcast, a write needs a full two-way handshake, `status 133` is Android's generic failure for that handshake breaking partway). Also reconfirmed the stale-peer-UUID artifact from §7/§12: one peer ID per phone never succeeded at any RSSI all session, a leftover from address rotation, unconditionally dead regardless of range — discounted from the range analysis. Written up in `PHASE0_MESH_FINDINGS.md` §13. | Still not a controlled measurement — distance was not walked or marked. Real Day 4 outdoor re-run still needed with actual distance markers. |
| | | | |
